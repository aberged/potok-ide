defmodule PotokIdeWeb.ConnCase do
  @moduledoc """
  ...
  by setting `use PotokIdeWeb.ConnCase, async: true`, although
  ...
  using do
    quote do
      ...
      import PotokIdeWeb.ConnCase
    end
  end

  setup tags do
    PotokIde.DataCase.setup_sandbox(tags)
  ...
  def register_and_log_in_account(%{conn: conn} = context) do
    ...
    scope = PotokIde.Accounts.Scope.for_account(account)
  ...
  def log_in_account(conn, account, opts \\ []) do
    token = PotokIde.Accounts.generate_account_session_token(account)
  ...
  defp maybe_set_token_authenticated_at(token, authenticated_at) do
    PotokIde.AccountsFixtures.override_token_authenticated_at(token, authenticated_at)
  end
end
