defmodule PotokIdeWeb.InvitationLive.Index do
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      <div class="space-y-6">
        <.header>
          Invitations
          <:subtitle>Pending group invitations for your current profile.</:subtitle>
        </.header>
        
        <div :if={@invitations == []} class="text-base-content/70">No pending invitations.</div>
        ...
        <div :for={inv <- @invitations} class="card bg-base-200">
          <div class="card-body">
            ...
            <div class="text-sm text-base-content/70">
              Invited by: <span class="font-semibold">{inv.inviter.username}</span>
            </div>
            
            <div class="mt-2">
              <.button phx-click="accept" phx-value-id={inv.id} variant="primary">Accept</.button>
            </div>
          </div>
        </div>
        
        <div><.link navigate={~p"/groups"} class="link">Back to Root Group</.link></div>
      </div>
    </Layouts.app>
    """
  end
  ...
  def handle_event("accept", %{"id" => id}, socket) do
    ...
    with %{} = invitation <- Social.get_pending_invitation_for_invitee(invitee, invitation_id),
      ...
      nil ->
        {:noreply, put_flash(socket, :error, "Invitation not found (or already accepted).")}
