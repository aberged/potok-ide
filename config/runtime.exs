...
ios_app_links = Application.get_env(:potok_ide, :ios_app_links, [])
...
ios_app_links =
  ...
  |> then(fn config ->
    case System.get_env("IOS_APP_LINK_APP_ID") do
      value when is_binary(value) and value != "" ->
        Keyword.put(config, :app_id, String.trim(value))
  ...
  |> then(fn config ->
    case System.get_env("IOS_APP_LINK_TEAM_ID") do
      value when is_binary(value) and value != "" ->
        Keyword.put(config, :team_id, String.trim(value))
  ...
  |> then(fn config ->
    case parse_env_list.(System.get_env("IOS_APP_LINK_PATHS")) do
      nil -> config
      paths -> Keyword.put(config, :paths, paths)
    end
  end)

config :potok_ide, :ios_app_links, ios_app_links
      ...
      "gmail" ->
        ...
        |> then(fn config ->
          config =
            config
            |> then(fn config ->
              if present?.(gmail_access_token),
                do: Keyword.put(config, :access_token, gmail_access_token),
                else: config
            end)
            |> then(fn config ->
              if present?.(gmail_client_id),
                do: Keyword.put(config, :client_id, gmail_client_id),
                else: config
            end)
            |> then(fn config ->
              if present?.(gmail_client_secret),
                do: Keyword.put(config, :client_secret, gmail_client_secret),
                else: config
            end)
            |> then(fn config ->
              if present?.(gmail_refresh_token),
                do: Keyword.put(config, :refresh_token, gmail_refresh_token),
                else: config
            end)

          if present?.(gmail_access_token) or
               Enum.all?([gmail_client_id, gmail_client_secret], present?) do
            config
          else
            raise """
